...
<div class="page-header">
	<h5>
<!-- 	操作权限管理 --><spring:message code="l_operating_authorization_management"/>
	</h5>
</div>
<style type="text/css">
...
<!-- 				组织结构 --><spring:message code="l_organization"/>
				</h4>
			</div>
...
<!--         操作授权 --><spring:message code="l_operating_authorization"/>
        </h4>
      </div>
...
jQuery(function($){
	...
	function getCheckData(treeId) {
	    ...
	    for(var i=0;i<nodes_array.length;i++){
	    	if(nodes_array[i].checked==true){
				treeChecked.push(nodes_array[i].id);
	    	}
	    }
	    return treeChecked;
	}
	...
	$('#authSubmit').click(function(){
		...
		$.ajax({
			url: '/resources/update.html',
			type: 'POST',
			data:{'userId': row.id,'resourcesIds':_a+""},
		})
		.done(function(data) {
			if(data.code==1){
	...
	$('#authModal').on('show.bs.modal', function (e) {
		var ztree2 = new loadZtree({
			...
			url:'/resources/query.html?userId='+row.id,
			check:{
				enable:true
			},
			_callback:function(event, treeId, treeNode){
			...
			_loadZTreeSuccessCallback:function(data){ //加载节点数成功时的回调函数
				...
				if(_data.length>0){
					for(var i=0;i<_data.length;i++) {
						treeObj.checkNode(treeObj.getNodeByParam("id",_data[i]),true);
					}
				}
			}
		});
...
// 		colNames:['id','人员编号','姓名','登记账号','所属部门','所属机构','人员类型','操作'],
		...
		colModel:[
			...
			{name:'no',index:'no',width:150},
			{name:'name',index:'name',width:100},
			{name:'username',index:'username',width:200},
			{name:_departmentName,index:_departmentName,width:200},
			{name:_orgName,index:_orgName,width:150},
			{name:'title',index:'title',width:150},
			{name:'editor',index:'editor',formatter:function(cellvalue, options, rowObject){
				var temp = '<div class="btn-group"><button class="btn btn-xs btn-primary viewBtn" data-toggle="modal" data-target="#authModal"><i class="icon-eye-open bigger-120"></i></button></div>';
					return temp;
			}}
		],
