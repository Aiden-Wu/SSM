...
(function($ , undefined) {
	...
	var Tree = function (element, options) {
		...
		this.$element.on('click', '.'+options["open-icon"], $.proxy( function(ev) { this.selectFolder(ev.currentTarget, ev); }, this));
		this.$element.on('click', '.'+options["close-icon"], $.proxy( function(ev) { this.selectFolder(ev.currentTarget, ev); }, this));
		this.$element.on('click', '.tree-folder-header', $.proxy( function(ev) { this.selOrUnAllChild(ev.currentTarget, ev); }, this));
		this.render();
	};
	...
	Tree.prototype = {
		...
		selOrUnAllChild : function(el, ev){
			if(ev) ev.stopPropagation();			
			var $el = $(el);
			var className = $el.attr("class");
			var content = $el.next(".tree-folder-content");
			var item = content.find(".tree-item");
			var itemh = content.find(".tree-folder-header");
			if(className.indexOf("tree-h-selected") == -1){
				if(content.html() != ""){//已经初始化
					$el.addClass("tree-h-selected");
					item.addClass("tree-selected");
					itemh.addClass("tree-h-selected");
				} else {
					//未初始化
				}
			} else {
				$el.removeClass("tree-h-selected");
				item.removeClass("tree-selected");
				itemh.removeClass("tree-h-selected");
				$el.css("background-color","");
			}
			//$el.find(".tree-item").trigger("click");
		}
	};
