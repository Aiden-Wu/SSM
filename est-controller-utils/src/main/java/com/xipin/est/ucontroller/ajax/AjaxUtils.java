package com.xipin.est.ucontroller.ajax;
...
import com.xipin.est.ucontroller.exception.system.SystemException;
import com.xipin.est.utils.param.Param;
import com.xipin.est.utils.rslt.type.APIResult;
...
//		response.addHeader("Access-Control-Allow-Headers","Origin, X-Requested-With, Content-Type, Accept");
	...
	public static void writeJSONP(HttpServletResponse response, Result rslt){
		...
		try {
			...
			JSONObject info = JSONObject.fromObject(rslt, config);
			out = response.getWriter();
			out.print("<head><meta http-equiv=\"Content-Type\" content=\"text/html; charset=utf-8\" /></head><script>/*document.domain='" + "//TODO" + "';*/</script><div id='jsonData'>"+info+"</div>");
			out.flush();
			out.close();
		} catch (IOException e) {
			throw new SystemException("write failed.", e);
